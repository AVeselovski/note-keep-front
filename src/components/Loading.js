		...
		return (
			<div className="loading-container">
				...
				<div className={`loading${this.props.alternative ? ' alternative' : ''}`}>
					<span
						className={`loading-dot dot-1${
							this.state.dots[0] === 'hidden' ? ' hidden' : ' visible'
						}`}
					/>
					<span
						className={`loading-dot dot-2${
							this.state.dots[1] === 'hidden' ? ' hidden' : ' visible'
						}`}
					/>
					<span
						className={`loading-dot dot-3${
							this.state.dots[2] === 'hidden' ? ' hidden' : ' visible'
						}`}
					/>
				</div>
			</div>
